...
dropout_fn = nn.Dropout2d # do not use if Torch v = 1.11 or 1.12
...
parser.add_argument('--epochs', default=100,
                    type=float, help='Training epochs')
# Dataset
parser.add_argument('--dataset', default='cifar10',
                    choices=['mnist', 'cifar10'], type=str, help='Dataset')
parser.add_argument('--grayscale', action='store_true',
                    help='Use grayscale CIFAR10')
# Dataloader
        ...
        for _ in range(n_layers):
            self.s4_layers.append(
                S4D(d_model, dropout=dropout,
                    transposed=True, lr=min(0.001, args.lr))
            )
...
def train():
    ...
    for batch_idx, (inputs, targets) in pbar:
        ...
        pbar.set_description(
            ...
            (batch_idx, len(trainloader), train_loss /
...
def eval(epoch, dataloader, checkpoint=False):
    ...
    with torch.no_grad():
        ...
        for batch_idx, (inputs, targets) in pbar:
            ...
            pbar.set_description(
                'Batch Idx: (%d/%d) | Loss: %.3f | Acc: %.3f%% (%d/%d)' %
                (batch_idx, len(dataloader), eval_loss /
                 (batch_idx+1), 100.*correct/total, correct, total)
            )
